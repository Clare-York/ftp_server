...
class FTPHandler(AsyncChat):
    ...
    def ftp_STOR(self, file, mode='w'):
        ...
        try:
            ...
            if system == "Windows":
                filename = file.split("\\")[-1]  # 取文件名
                pathname = file.split("\\")[-2]  # 取文件上传目录
                check = homedir.split("\\")[-1]  # 取配置的根目录
            else:
                filename = file.split("/")[-1]
                pathname = file.split("/")[-2]
                check = homedir.split("/")[-1]
            if pathname == check:
                # 如果上传到根目录，就自动以当日日期创建文件夹，并存放在该文件夹内
                dirname = datetime.now().strftime('%Y-%m-%d')
                dirpath = os.path.join(homedir, dirname)
                if not os.path.exists(dirpath):
                    os.mkdir(dirpath)
                file = os.path.join(dirpath, filename)  # 重定向文件上传位置
